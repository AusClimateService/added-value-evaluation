...
def rcm_model_loader(gcm, rcm, scen, freq, var):
    if rcm == "BARPA-R":
        ...
        if gcm == "ERA5":
            scen = "evaluation"
        return ccam_drs_interface.get_ccam_files(gcm, scen, freq, var), var
    else:
...
def loop_av(args, gcm_files, rcm_files, obs_files, gcm_varname, rcm_varname, obs_varname, ofile, ifile_X_gcm=None, ifile_X_rcm=None, ifile_X_obs=None, agcd_mask=False, reuse_X=True, upscale2gcm=False, upscale2ref=False, ifile_ref_grid=""):
    ...
    --ofile {ofile}
    --ifile-X-gcm {ifile_X_gcm} --ifile-X-rcm {ifile_X_rcm} --ifile-X-obs {ifile_X_obs}
...
def loop_pav(args, gcm_hist_files, gcm_fut_files, rcm_hist_files, rcm_fut_files, gcm_varname, rcm_varname, ofile, reuse_X=False, agcd_mask=False,
    ...
    --lon0 {args.lon0} --lon1 {args.lon1}
    --ofile {ofile}
    --ifile-X-gcm-hist {ifile_X_gcm_hist} --ifile-X-gcm-fut {ifile_X_gcm_fut} --ifile-X-rcm-hist {ifile_X_rcm_hist} --ifile-X-rcm-fut {ifile_X_rcm_fut}
    --log-level {args.loglevel}
    """
